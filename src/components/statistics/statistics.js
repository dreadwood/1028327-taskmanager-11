...
export default class Statistics extends AbstractSmartComponent {
  ...
  getTemplate() {
    const placeholder = this._createPlaceholder(this._dateFrom, this._dateTo);
    const tasksCount = this._getTasksByDateRange(this._tasks.getTasks(), this._dateFrom, this._dateTo).length;
    return (
  ...
  _createPlaceholder(dateFrom, dateTo) {
    const format = (date) => {
  ...
  _getTasksByDateRange(tasks, dateFrom, dateTo) {
  ...
  _renderDaysChart(daysCtx, tasks, dateFrom, dateTo) {
    const days = this._calculateBetweenDates(dateFrom, dateTo);
  ...
  _calculateBetweenDates(from, to) {
    const result = [];
  ...
  _getUniqItems(item, index, array) {
    return array.indexOf(item) === index;
  }

  _calcUniqCountColor(tasks, color) {
